...
const PlantOfDay = () => {
  const isInitialized = useRef<boolean>(false); // todo : Remove at production
  ...
  useEffect(() => {
    if (!isInitialized.current) {
      ...
      if (typeof window !== "undefined") {
        ...
        const imgWidth = Math.floor((vw - 232) * 0.4 - 24);
        ...
        {Pod.plant.common_name != "" ? (
          <>
            ...
            <div className="h-full w-[40%] bg-custom-green rounded-xl flex items-center flex-col p-4 shadow-md">
              ...
              <div className="bg-white text-gray-900 text-[16px] w-full flex-grow rounded-md p-2 font-medium text-justify">
                {TextShorter(Pod.plant.description, 350)}
              </div>
              <Link href={UrlGen("plants", Pod.plant.scientific_name)}>
                <button className="bg-custom-blue-green text-white text-[16px] font-semibold mt-4 py-2 px-4 rounded-md">
                  Learn more...
                </button>
              </Link>
            </div>
          ...
          <>
            ...
            <div className="h-full w-[40%] bg-custom-green rounded-xl flex items-center flex-col p-4 shadow-md">
              <div role="status" className="w-full animate-pulse">
                <div className="w-full h-4 flex justify-center mb-2.5">
