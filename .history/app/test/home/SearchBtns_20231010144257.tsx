...
export const SideSearchBtn: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sideSerachBtn = document.getElementById("sideSearchBtn");
      var vh = window.innerHeight;
      var scrollHeight = window.scrollY;
      if (scrollHeight >= vh * 0.2) {
        console.log("scroll ok");
        sideSerachBtn?.classList.add("animate-sideSearchBtn");
      }
    };
  ...
  return (
    <>
      <div
        id="sideSearchBtn"
        className="fixed bottom-[60px] right-0 opacity-0 rounded-full bg-custom-blue-green shadow-xl w-[50px] h-[50px] flex items-center justify-center cursor-pointer"
        onClick={openSearch}
      >
